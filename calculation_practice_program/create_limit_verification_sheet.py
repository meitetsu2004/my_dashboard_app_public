...
# --- 設定項目 ---
PROBLEMS_FILE = 'problems_limit.json'
OUTPUT_FILENAME_BASE = '極限問題解答一覧'
...
"""
    ...
    for p in problems:
        ...
        # 極限の数式は見やすさのため displaystyle を適用すると良いが、
        # 「全く同じ体裁」という要望を守り、単純な置換にとどめます。
        question_formula = p['question'].replace('$$', '$')
        answer_formula = p['answer'].replace('$$', '$')
        
        left_cell = f"\\textbf{{[{problem_id}] (難易度: {difficulty})}} \\\\ {question_formula}"
        right_cell = answer_formula
        
        # 行ごとに独立した表を作成して改ページに対応させる方式
        body += (
            f"\n\\vspace{{1mm}}\n"
            f"\\noindent\\begin{{tabular*}}{{\\linewidth}}{{@{{\\extracolsep{{\\fill}}}} p{{0.48\\linewidth}} p{{0.48\\linewidth}} @{{}}}}\n"
